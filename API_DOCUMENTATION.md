...
2. **토큰 관리**: Access Token은 메모리에만 저장, Refresh Token은 HttpOnly 쿠키로 관리
3. **이미지 업로드**: Base64 인코딩된 문자열 또는 MultipartFile 지원
4. **모임 카테고리**: 10가지 기본 모임 카테고리 제공
5. **온보딩**: 로그인 후 온보딩 완료 여부에 따라 리다이렉트 처리 필요
6. **데이터베이스**: `user_interests` 테이블을 사용하여 사용자-카테고리 매핑 관리
7. **보안**: JWT 필터에서 공개 엔드포인트는 인증 검증 건너뛰기
8. **OAuth2**: 소셜 로그인 시 Access Token을 URL 파라미터로 전달
